...
function getAllData(page,pageIndex) {
    const absoluteUrl = new URL(`${externalDataSourceHost}/${endPoint}?page=${page}&pageSize=${pageIndex}`).href;
...
function getById(id) {
    const absoluteUrl = new URL(`${externalDataSourceHost}/${endPoint}/${id}`).href;
    let prom = new Promise((resolve, reject)=>{
        fetch(absoluteUrl)
            .then((response) => {
                if (!response.ok) {
...
function postData(data) {
    const absoluteUrl = new URL(`${externalDataSourceHost}/${endPoint}`).href;
...
function putData(objectId,data) {
    const absoluteUrl = new URL(`${externalDataSourceHost}/${endPoint}/${objectId}`).href;
    let prom = new Promise((resolve, reject)=>{
        fetch(absoluteUrl, {
            method: 'PUT',
            headers: {
...
templateRouter.get('/:objectDefinitionExternalReferenceCode',async(req,res)=>{
...
templateRouter.post('/:objectDefinitionExternalReferenceCode',async(req,res)=>{
...
templateRouter.get('/:objectDefinitionExternalReferenceCode/:externalReferenceCode',async(req,res)=>{
    const { companyId, languageId, scopeKey,userId,page,pageSize } = req.query;
...
templateRouter.put('/:objectDefinitionExternalReferenceCode/:externalReferenceCode',async(req,res)=>{
    let {companyId,languageId,scopeKey,userId,objectEntry} = req.body;
...
templateRouter.delete('/:objectDefinitionExternalReferenceCode/:externalReferenceCode',async(req,res)=>{
    let {companyId,languageId,scopeKey,userId,objectEntry} = req.body;
    let objectId = req.params.externalReferenceCode;
